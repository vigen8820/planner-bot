...
from telegram import (
    ...
    BotCommand, ReplyKeyboardMarkup, ReplyKeyboardRemove, KeyboardButton
)
...
MAIN_KB = ReplyKeyboardMarkup(
    [
        ...
        [KeyboardButton("💸 Сброс финансов"),    KeyboardButton("✖️ Закрыть меню")],
    ],
...
async def setup_commands(app: Application):
    await app.bot.set_my_commands([
        ...
        BotCommand("menu",          "Показать меню кнопок"),
...
async def cmd_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Показать меню если оно было скрыто."""
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if text == BTN_CLOSE:
        await update.message.reply_text(
            "Меню скрыто. Открыть снова — /menu или /start",
